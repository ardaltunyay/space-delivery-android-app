...
import com.github.spacedelivery.androidapp.data.local.createSpaceStationeDao
...
val appModule = module {
    ...
    single {
        Room.databaseBuilder(androidContext(), AppDatabase::class.java, "SpaceDelivery-Db")
            .fallbackToDestructiveMigration().build()
    }
    single { createSpaceVehicleDao(appDatabase = get()) }
    single { createSpaceStationeDao(appDatabase = get()) }
